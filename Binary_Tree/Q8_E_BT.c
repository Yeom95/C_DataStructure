...
int hasGreatGrandchild(BTNode *node)
{
    ...
    int findGreatGrandChild = 0;
    int countLeft = hasGreatGrandchild(node->left);
    int countRight = hasGreatGrandchild(node->right);
    ++findGreatGrandChild;

    findGreatGrandChild += countLeft > countRight ? countRight : countLeft;

    if(findGreatGrandChild > 2)
        printf(" %d ",node->item);

    return findGreatGrandChild;
}
