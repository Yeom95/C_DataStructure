...
void levelOrderTraversal(BSTNode* root)
{
	...
	while(levelOrderQueue.head != NULL){
		BSTNode *targetNode = dequeue(&(levelOrderQueue.head),&(levelOrderQueue.tail));

		if(targetNode->left != NULL)
			enqueue(&(levelOrderQueue.head),&(levelOrderQueue.tail),targetNode->left);
		if(targetNode->right != NULL)
			enqueue(&(levelOrderQueue.head),&(levelOrderQueue.tail),targetNode->right);

		printf("%d ",targetNode->item);
	}
}
